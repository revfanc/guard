...
首次创建时会给 base 与 sentinel 写入同一 generation，并只增加一条同 URL sentinel；刷新后重新创建 Guard 会直接接管当前 sentinel。同一 `window` 中的重复模块或 HMR 实例共享一个 Runtime。用户单步返回后，库恢复保护并调用 handler：
...
`dispose()` 不会替业务导航；它只清除本库自己的保护。如果历史已经由外部改变，它会正常结束且不改写新位置。快速 unmount → mount 时，可以在旧 Promise 完成前创建下一代 Guard，runtime 会将它排队并在精确 base `popstate` 后激活。
